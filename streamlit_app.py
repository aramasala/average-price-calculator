...
if calculate:
    try:
        ...
        with st.expander("📝 Show Detailed Calculation"):
            st.latex(rf"""
            \text{{Average Price}} = \frac{{({initial_qty} \times {initial_price}) + ({new_qty} \times {new_price})}}{{{initial_qty} + {new_qty}}}
            """)
            
            st.code(f"""
            Initial Investment = {initial_qty} × {initial_price} = {initial_qty * initial_price:.{precision}f}
            New Investment = {new_qty} × {new_price} = {new_qty * new_price:.{precision}f}
...
# Footer
st.markdown("---")
st.caption("Built with Python, Pydantic, and Streamlit | [GitHub](https://github.com/aramasala/average-price-calculator)")